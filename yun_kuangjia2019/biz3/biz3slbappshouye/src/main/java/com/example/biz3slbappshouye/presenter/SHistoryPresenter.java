...
public class SHistoryPresenter extends Presenter<SListCommView> {
    public static String TAG = SHistoryPresenter.class.getSimpleName();

    public void getHistoryData(int page, int limit) {
...
//        requestData.put("params", JSON.toJSON(params));
        ...
        RetrofitNetNew.build(SIndexApi.class, getIdentifier()).get_index_history(BanbenUtils.getInstance().getVersion(), BanbenUtils.getInstance().getImei(), BanbenUtils.getInstance().getToken(), requestBody).enqueue(new Callback<ResponseSlbBean<SListCommBean>>() {
            @Override
