...
public class SLBBannerPresenter extends Presenter<SLBBannerView> {
    ...
    public void getLBBannerData(String bannerType) {
        JSONObject requestData = new JSONObject();
        requestData.put("bannerType", bannerType);// 0:首页 | 1:分类轮播
...
//        requestData.put("params", JSON.toJSON(params));
        ...
        RetrofitNetNew.build(SIndexApi.class, getIdentifier()).get_index_tingshulunbo(BanbenUtils.getInstance().getVersion(), BanbenUtils.getInstance().getImei(), BanbenUtils.getInstance().getToken(), requestBody).enqueue(new Callback<ResponseSlbBean<SLBBannerBean>>() {
            @Override
            public void onResponse(Call<ResponseSlbBean<SLBBannerBean>> call, Response<ResponseSlbBean<SLBBannerBean>> response) {
                ...
                if (response.body().getCode() != 0) {
                    getView().OnLBBannerNodata(response.body().getMsg());
                    return;
                }
                getView().OnLBBannerSuccess(response.body().getData());
                call.cancel();
            }
