...
public class SListenBookPresenter extends Presenter<SListCommView> {

    public static String TAG = SListenBookPresenter.class.getSimpleName();

    public void getListenBookData(int page, int limit) {
...
//        requestData.put("params", JSON.toJSON(params));
        ...
        RetrofitNetNew.build(SIndexApi.class, getIdentifier()).get_index_gxjd(BanbenUtils.getInstance().getVersion(), BanbenUtils.getInstance().getImei(), BanbenUtils.getInstance().getToken(), requestBody).enqueue(new Callback<ResponseSlbBean<SListCommBean>>() {
            @Override
