...
import com.example.biz3slbappusercenter.presenter.SUpdateUserInfoPresenter;
...
//        String b = tv_birthday2.getText().toString().trim();
    ...
    public void OnUpdateUserInfoSuccess(String s) {
        SPUtils.getInstance().put(CommonUtils.USER_NAME, edt1.getText().toString().trim());
        onLoginSuccess();
    }
...
//            }
        ...
        if (TextUtils.equals("1", SPUtils.getInstance().getString(CommonUtils.USER_FORCE_CHILD)) && !bean.getPbUser().isHasChild()) {
            iv1.setVisibility(View.INVISIBLE);
        } else {
            iv1.setVisibility(View.VISIBLE);
        }
