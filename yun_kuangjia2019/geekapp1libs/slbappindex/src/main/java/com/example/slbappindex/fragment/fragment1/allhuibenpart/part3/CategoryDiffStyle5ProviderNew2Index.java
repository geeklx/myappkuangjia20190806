...
import com.example.slbappcomm.ratingstarview.ConvertUtil;
import com.example.slbappcomm.ratingstarview.RatingStarView;
...
public class CategoryDiffStyle5ProviderNew2Index extends BaseItemProvider<CategoryDiffBeanNew2Index, BaseViewHolder> {

    @Override
    public int viewType() {
        return CategoryDiffAdapterNew2Index.STYLE_FIVE;
    }

    @Override
    public int layout() {
        return R.layout.activity_fragment21_imgstyle3_item_new1;
    }
    ...
    public void convert(BaseViewHolder helper, CategoryDiffBeanNew2Index data, int position) {
        ...
        Glide.with(mContext).load(data.getmBean().getCoverImg())
                .apply(options)
                .into(iv);
        RatingStarView rsv_rating = helper.itemView.findViewById(R.id.rsv_rating);
        float yeatInt = ConvertUtil.convertToFloat("1.5", 1.0f);
        MyLogUtil.e("--yunlx--",yeatInt+"");
        rsv_rating.setRating(yeatInt);
