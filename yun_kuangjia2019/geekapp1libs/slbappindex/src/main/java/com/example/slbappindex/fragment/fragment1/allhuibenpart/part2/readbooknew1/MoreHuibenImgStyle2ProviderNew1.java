...
public class MoreHuibenImgStyle2ProviderNew1 extends BaseItemProvider<MoreHuibenImgBeanReadBook, BaseViewHolder> {

    @Override
    public int viewType() {
        return MoreHuibenImgAdapterNew1.STYLE_TWO;
    }
    ...
    public void convert(BaseViewHolder helper, MoreHuibenImgBeanReadBook data, int position) {
        //加载图片
        File file = new File(data.getmBean().getBgImg());
...
//                Glide.with(mContext).load(item.getCornerMap().getP4().getImgUrl()).into(iv_bktt1);
        ...
        helper.setText(R.id.tv_provider31, data.getmBean().getDescr());
