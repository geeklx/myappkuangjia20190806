...
public class MoreHuibenImgStyle2Provider extends BaseItemProvider<MoreHuibenImgBean, BaseViewHolder> {

    @Override
    public int viewType() {
        return MoreHuibenImgAdapter.STYLE_TWO;
    }
    ...
    public void convert(BaseViewHolder helper, MoreHuibenImgBean data, int position) {
        //加载图片
        File file = new File(data.getmBean().getBannerImg());
        ...
        Glide.with(mContext).load(data.getmBean().getBannerImg())
                .apply(options)
                .into(iv);
...
//                Glide.with(mContext).load(item.getCornerMap().getP4().getImgUrl()).into(iv_bktt1);
        ...
        helper.setText(R.id.tv_provider3, data.getmBean().getBookName());
        helper.setText(R.id.tv_provider31, data.getmBean().getBookName());
